(function(){
  ...
  function injectStyle(){
    ...
    style.textContent=`
      ...
      .vrb-mobile-note{position:fixed;left:max(14px,env(safe-area-inset-left));bottom:calc(16px + env(safe-area-inset-bottom));z-index:1180;width:44px;height:44px;border:1px solid var(--line);border-radius:14px;background:var(--card);color:var(--text);box-shadow:0 10px 32px rgba(17,18,20,.14);display:none;place-items:center;opacity:0;transform:translateY(8px);pointer-events:none;transition:opacity var(--dur-state,220ms) ease,transform var(--dur-state,220ms) var(--ease-velnar,cubic-bezier(.23,1,.32,1))}.vrb-mobile-note svg{width:19px;height:19px}.vrb-mobile-note.has-note{color:var(--accent)}.vrb-mobile-note.visible{opacity:1;transform:translateY(0);pointer-events:auto}html.vr-reader-mini-active .vrb-mobile-note{bottom:calc(78px + env(safe-area-inset-bottom))}
      ...
      @media(max-width:640px){.vrb-note-sheet{width:100vw;border-left:0}.vrb-discuss{padding-left:10px!important;padding-right:10px!important}.vrb-discuss span{display:none}.vrb-discuss svg{width:17px;height:17px}.vrb-mobile-note{display:grid}}
      @media(prefers-reduced-motion:reduce){.vrb-note-layer,.vrb-note-sheet,.vrb-selection-btn,.vrb-mobile-note{transition:none!important;transform:none!important}.vrb-note-layer:not(.open) .vrb-note-sheet{display:none}}
    `;
  ...
  function mountArticleBridge(){
    ...
    const mobileNote=document.createElement('button');mobileNote.id='vrbMobileNote';mobileNote.className='vrb-mobile-note';mobileNote.type='button';mobileNote.setAttribute('aria-label','随时打开阅读笔记');mobileNote.setAttribute('title','阅读笔记');mobileNote.innerHTML=NOTE_ICON;document.body.appendChild(mobileNote);
    ...
    function updateMobileNote(){mobileNote.classList.toggle('visible',matchMedia('(max-width:640px)').matches&&scrollY>420&&!layer.classList.contains('open'))}
    function openPanel(){renderPanel();layer.classList.add('open');layer.setAttribute('aria-hidden','false');updateMobileNote();setTimeout(()=>noteText.focus(),80)}
    function closePanel(){layer.classList.remove('open');layer.setAttribute('aria-hidden','true');updateMobileNote()}

    noteBtn.addEventListener('click',openPanel);
    mobileNote.addEventListener('click',openPanel);
    window.addEventListener('scroll',updateMobileNote,{passive:true});window.addEventListener('resize',updateMobileNote,{passive:true});updateMobileNote();
  ...
  function updateNoteState(){
    const active=hasNote(articleId());
    const btn=document.getElementById('vrbNoteBtn');if(btn)btn.classList.toggle('has-note',active);
    const mobile=document.getElementById('vrbMobileNote');if(mobile)mobile.classList.toggle('has-note',active);
  }
